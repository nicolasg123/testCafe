...
export class Local implements IBasicTest {
  public meta: TestMeta = {
    ID: 'Localsuite',
    SEVERITY: TestMetaSeverity.blocker,
    STORY: 'Localsuite',
    TEST_RUN: 0,
    SAUCE_JOB: 'Localsuite',
  }
  ...
  public async runTest(): Promise<any> {
    ...
    const responseName : string = `RespName${id}`.replace(/-/g, '')
     ...
     await this.testCaseRunner.when('login Localy', async () => {
      await BatNavigator.loginNewUILocally()
     }).it('login to new locally ends successfully')
    
     await this.testCaseRunner.when('check landing', async () => {
     await AFMLanding.checkLandingPageLocal()
    }).it('landing ends successfully')

    await this.testCaseRunner.when('Create Simple Monitor', async () => {
      await SimpleMonitorCreator.createLocal(testNameAdv,endpointURL)  
    ...
    await this.testCaseRunner.when('hit manage locations', async () => {
      await MonitoringLanding.manageLocations()
    }).it('manage location sections load sucessfully')
    ...
    await this.testCaseRunner.when('Create monitor with reporters', async () => {
      await MonitoringLanding.createMonitorReporters(testNameRep,endpointURL,'PagerDuty','Routing key')
    }).it('New monitor with reporter pd added')

    await this.testCaseRunner.when('Create monitor with reporters', async () => {
      await MonitoringLanding.createMonitorReporters(testNameRep,endpointURL,'NewRelic','License key')
    ...
    }).it('New Schedule created on private location ')
